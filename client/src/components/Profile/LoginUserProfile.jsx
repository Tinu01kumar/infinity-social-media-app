...
const LoginUserProfile = () => {
 ...
 const navigate=useNavigate();
  console.log(usernameFromParmas, "###############");
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/profile-details`, {
        params: {
 ...
 const fetchMoreData=async ()=>{
  try{
    ...
    console.log(response , "########")
    setShowpost((prevPosts)=> [...prevPosts , ...response.data.postsWithDetails])
    setLoading(false);
  }catch(error)
  {
...
setbannerloading(true)
  ...
  const postDelete = async (postid) => {
    const res = await axios.post(`${process.env.REACT_APP_API_URL}/deletepost`, {
      postid,
    });
    if (res.data.status) {
      setShowpost((prevPosts)=>prevPosts.filter((post)=>post.id!==postid))
    }
  };
  ...
  const handleDeletecomment = async (commentid, postid) => {
    try {
      ...
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/deletecomment`, {
        commentid,
        postid,
      });
      if (res.data.status) {
    ...
    <div>
      ...
      <ProfileContainer>
        <EditModal style={{ display: showProfileEditModal ? "block" : "none" }}>
          ...
          <ButtonSection>
          {bannerloading ? (<div style={{ textAlign: "center", marginTop: "" }}>
          <CircularProgress />
        </div>): (<>
  ...
  {bannerloading ? (
    ...
    <>
      ...
      <div>
        <ProfileContent name={name} differentusername={Cookies.get("username")} />
        <Followconcept>
          ...
          {usernameFromParmas === Cookies.get("username") ? (
            <Link to={`/profiles/${usernameFromParmas}/mode`}>Mode</Link>
          ) : null}
        </Followconcept>
      </div>
      {usernameFromParmas === Cookies.get("username") ? (
          ...
          <PostBox key={index}>
            <PostContent>
              ...
              >
                ...
                <OptionsContainer>
                  <Dot onClick={() => toggleOptions(post.id)}>
                    <h5>...</h5>
                  </Dot>
                    ...
                    <OptionsDiv
                      ...
                      showOptions={showOptions[post.id]}
                      ref={optionsRef}
                      onClick={() => toggleOptions(post.id)}
                    >
                      <div onClick={() => postDelete(post.id)}>Delete</div>
                      <div onClick={() => postupdate(post.id)}>Update</div>
                    </OptionsDiv>
              ...
              <Description expand={showFullDescription[post.id]}>
                ...
                {post.description.length > 100 && (
                  <SeeMoreLink onClick={() => toggleDescription(post.id)}>
                    {showFullDescription[post.id] ? "See less" : "See more"}
                  </SeeMoreLink>
                )}
              </Description>
            ...
            <CommentAndLikeSection>
              ...
              <CommentButton onClick={() => toggleCommentSection(post.id)}>
